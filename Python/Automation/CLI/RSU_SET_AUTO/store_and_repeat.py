...
def config_store_and_repeat(master,app):
	global toggle
	toggle	= not toggle
	...
	if toggle:
		master.sendline("streaming ipaddr "+str(inet_ntoa(pack('>I',randint(0,0xffffffff)))))
	else:
		master.sendline("streaming ipaddr "+str(IPAddress(getrandbits(128))))
	sleep(1)
        master.expect("#")
...
def validate_store_and_repeat(master,slave_set,app):
	...
	master.sendline("streaming ipaddr")
	sleep(1)
	...
	for slave in slave_set:
			...
			slave.sendline("streaming ipaddr")
			sleep(1)
