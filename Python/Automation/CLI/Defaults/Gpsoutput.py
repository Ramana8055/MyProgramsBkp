...
try:
	...
	child.logfile_read=open("GpsOutputStatus.txt",'wb')
	...
	child.sendline("sample-rate "+str(sample_rate))
	child.expect("#")
    ...
    if not exit_flag:
	stdout.flush()
	system("cat GpsOutputStatus.txt | grep -A1 -B2 -Ei 'failed|invalid|\^command|parameter|timedout' > GpsOutputError.txt")
	system("if [ `cat GpsOutputError.txt|wc -l` -eq 0 ] ; then rm GpsOutputError.txt ; fi")
	exit(0)
    else:
	exit(1)
except:
