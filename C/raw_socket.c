...
static int parse_raw_hex(char *buf, uint8_t *txbuf, int txbuf_len)
{
    char *ptr;
    int  i;
    int  len;
...
int main(int argc, char **argv)
{
    ...
    rawpacket = (struct raw_packet *)txbuf;

    sock = socket(AF_PACKET, SOCK_RAW, htons(0x88fc));
    if (sock < 0) {
        perror("Socket\n");
        goto err2;
    }
    ...
    rawpacket->eth.ether_shost[0] = ifmac[0];
    rawpacket->eth.ether_shost[1] = ifmac[1];
    rawpacket->eth.ether_shost[2] = ifmac[2];
    rawpacket->eth.ether_shost[3] = ifmac[3];
    rawpacket->eth.ether_shost[4] = ifmac[4];
    rawpacket->eth.ether_shost[5] = ifmac[5];

    rawpacket->eth.ether_dhost[0] = 0xff;
    rawpacket->eth.ether_dhost[1] = 0xff;
    rawpacket->eth.ether_dhost[2] = 0xff;
    rawpacket->eth.ether_dhost[3] = 0xff;
    rawpacket->eth.ether_dhost[4] = 0xff;
    rawpacket->eth.ether_dhost[5] = 0xff;

    rawpacket->eth.ether_type = htons(0x88fc);
    ...
    while (1) {
        ...
        if (fgets(buf, sizeof(buf), fp)) {
            ...
            line ++;
            len = parse_raw_hex(buf, rawpacket->txbuf,
                    sizeof(txbuf) - sizeof(struct raw_packet));
            if (len <= 0) {
            ...
            if (debug)
                print_hex_dump(rawpacket->txbuf, len);

            ret = sendto(sock, rawpacket, sizeof(struct raw_packet) + len, 0,
