 ...
 */
static void scullc_do_deferred_op(struct work_struct *ws)
{
	struct async_work *stuff = container_of (ws, struct async_work, work);
...
#endif
printk(KERN_INFO "Scullc Inserted\n");
	return 0; /* succeed */
...
printk(KERN_INFO "Scullc removed\n");
}
